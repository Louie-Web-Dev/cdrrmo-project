...
<body>    
    ...
    <button class="srchbtn" id="search-btn">Search</button>
    <button class="srchbtn" id="clear-btn">Clear</button>
</div>
    <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
    ...
    $('#clear-btn').on('click', function () {
        ...
        $.ajax({
            ...
            function updateTable() {
                $.ajax({
                    type: 'GET',
                    url: 'get_archive_list.php', // Replace with the actual URL to fetch the latest incidents
    ...
    <table class="new-report-table">
        ...
        <div id="inspectModal" class="modal">
            <div class="modal-content">
                ...
                <button id="incidentDetailsBtn" target="_blank">Read Report</button>
        ...
        <div id="editReportModal" class="modal">
            <div class="modal-content">
                ...
                <button id="emsReportBtn" target="_blank">Edit EMS Report</button>
                <button id="finalemsReportBtn" target="_blank">Make Final EMS Report</button>
            </div>
        </div>
...
function changeStatusButton(incidentId, currentStatus) {
    ...
    // Show a confirmation dialog only if the new status is "Resolved"
    if (newStatus === 'Resolved') {
        // Reset confirmationShown to show the dialog for the next resolution
        confirmationShown = false;

        if (!confirm('Are you sure you want to resolve this incident?')) {
            // If the user cancels the confirmation, return from the function
            return;
        }
    ...
    if (newStatus === 'Verified') {
        ...
        if (!confirm('Are you sure you want to move back this to Incident List?')) {
            // If the user cancels the confirmation, return from the function
            return;
        }
...
updateTable();
    ...
    if (button) {
        button.text((newStatus === 'Verified') ? 'Resolve' : 'Undo');
    ...
    if (newStatus === 'Verified' || newStatus === 'Archive') {
...
// search button will stop interval
...
$(document).ready(function () {
    ...
    function updateTableWithSearchResults() {
        ...
        var url = 'get_archive_list.php?search=' + encodeURIComponent(searchQuery);
        ...
        $.ajax({
            ...
            complete: function () {
                ...
                var intervalDuration = searchQuery.trim() === '' ? 1000 : 60000;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var finalemsReportBtn = document.getElementById('finalemsReportBtn');
    ...
    incidentDetailsBtn.onclick = function () {
        var url = 'read_report.php?id=' + selectedIncidentId;
    ...
    finalemsReportBtn.onclick = function () {
        var url = 'ems_text_report.php?id=' + selectedIncidentId;
        window.open(url, '_blank'); // Open in a new tab
        editReportModal.style.display = 'none';
    };
