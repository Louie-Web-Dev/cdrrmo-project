...
$excludedColumns = array('id', 'Incident_ID', 'status', 'action_taken_others', 'closed_by', 'others', 'resolution_remarks', 'resolution_remarks', 'other', 'vehicle_type', 'number_affected', 'age', 'contact', 'othervictims1', 'othervictims2', 'other_dispatch');
...
$query = "SELECT *, CONCAT(date, ' ', time) AS datetime_combined FROM incidents 
          WHERE status = 'Resolved' AND (action_taken = 'Dispatch' OR action_taken = 'Both')
          AND (incident_type LIKE '%$searchQuery%' OR location LIKE '%$searchQuery%' OR Incident_ID LIKE '%$searchQuery%' OR date LIKE '%$searchQuery%') 
