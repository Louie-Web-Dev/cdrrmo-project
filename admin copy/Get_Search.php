...
$query = "SELECT *, CONCAT(date, ' ', time) AS datetime_combined FROM incidents WHERE status = 'Resolved' AND (incident_type LIKE '%$searchQuery%' OR location LIKE '%$searchQuery%' OR Incident_ID LIKE '%$searchQuery%' OR date LIKE '%$searchQuery%') ORDER BY datetime_combined DESC LIMIT $entries";
