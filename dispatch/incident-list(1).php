        ...
        <table class="new-report-table">
            <thead>
                <tr>
                    ...
                    <th>Status<br>(Completed)<br>(Accepted By)</th>
                    <th>Action</th>
        ...
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                if (newStatus === 'Verified') {
                    // If the status is 'Verified', fetch 'verified_by' from the database
                    fetchVerifiedByAndUpdateAcceptedBy(incidentId);
                } else {
                    // If the status is 'Resolved', update the status and button
...
function fetchVerifiedByAndUpdateAcceptedBy(incidentId) {
    console.log('Fetching verified_by for incidentId: ', incidentId);
    $.ajax({
        type: 'GET',
        url: 'get_full_name.php?incident_id=' + incidentId,
        success: function (response) {
            console.log('Fetched verified_by: ', response);
            // Update the 'accepted_by' field in the database
            updateAcceptedBy(incidentId, response);
        },
        error: function () {
            console.error('Failed to fetch verified_by. Please try again.');
        }
    });
}

function updateAcceptedBy(incidentId, verifiedBy) {
    console.log('Updating accepted_by for incidentId: ', incidentId, ' with verifiedBy: ', verifiedBy);
    $.ajax({
        type: 'POST',
        url: 'save_accepted_by.php',
        data: { incident_id: incidentId, verified_by: verifiedBy },
        success: function () {
            console.log('Accepted by updated successfully.');
            // Once 'accepted_by' is updated, update the status and button
            updateStatusAndButton(incidentId, 'Resolved');
        },
        error: function () {
            console.error('Failed to update accepted_by. Please try again.');
        }
    });
}
