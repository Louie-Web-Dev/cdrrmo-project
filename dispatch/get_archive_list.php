...
$query = "SELECT *, CONCAT(date, ' ', time) AS datetime_combined FROM incidents WHERE status = 'Archived' AND (incident_type LIKE '%$searchQuery%' OR location LIKE '%$searchQuery%' OR Incident_ID LIKE '%$searchQuery%' OR date LIKE '%$searchQuery%' OR verified_by LIKE '%$searchQuery%') ORDER BY datetime_combined DESC LIMIT $entries";
...
while ($row = $result->fetch_assoc()) {
    ...
    $tableBody .= '<td class="status-cell" data-incident-id="' . $row['id'] . '">' . $row['status'] . '<br>' . $row['verified_by'] . '</td>';
